...
//        List<AptTemp> all = aptTempRepository.findAll();
        List<String> buildingCodes = guavaBuildingRepository.findAll().stream().map(x -> x.getBuildingCode()).collect(Collectors.toList());
        List<AptTemp> byAptIdIn = aptTempRepository.findByAptIdNotIn(buildingCodes);
//        List<AptTemp> byAptIdIn = aptTempRepository.findByAptId("69tf6");
        return byAptIdIn;
//        return all.stream().filter(x -> buildingCodes.contains(x.getAptId())).collect(Collectors.toList());
    }
    ...
    public GuavaBuilding processBuilding(AptTemp aptTemp) {
        try {
            if(aptTemp.getData() != null) {
                JsonNode jsonNode = objectMapper.readValue(aptTemp.getData().substring(aptTemp.getData().indexOf("\"data"), aptTemp.getData().indexOf("nience")).replace("\"data\":{", "{").replace(",\"conve", "}"), SimpleType.constructUnsafe(JsonNode.class));
...
//            guavaBuilding.setRentalBusinessRatio(jsonNode.get("rental_business_ratio").intValue());
        ...
        if(guavaBuilding.getLng() != null && guavaBuilding.getLat() != null) {
